...
X_train = sequence.pad_sequences(seq, maxlen=22)
# test
...
y_test = y.astype(int)
X_test = sequence.pad_sequences(seq, maxlen=22)
# validation 
...
X_validation = sequence.pad_sequences(seq, maxlen=22)


X_train = X_train.reshape(1100, 22, 27, 13, 1)
X_test = X_test.reshape(200, 22, 27, 13, 1)
X_validation = X_validation.reshape(100, 22, 27, 13, 1)
...
model = Sequential(
    [
        Conv3D(16, (3, 3, 3), input_shape=(22, 27, 13, 1), 
...
tf.math.confusion_matrix(testing, model.predict(X_test).argmax(axis=1)+1)
